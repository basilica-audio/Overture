...
- **The suite now presents itself as Basilica Audio in every host.** `COMPANY_NAME` moves from
  `Yves Vogl` to `Basilica Audio`, so Overture files under the brand in Logic's plugin manager,
  ...
  `com.yvesvogl.overture` on purpose, because changing it is what would break existing projects, and
...
- **User presets now live under `Basilica Audio`, and the ones you already saved come with them.**
  The folder moves to `~/Library/Audio/Presets/Basilica Audio/Overture/` (macOS) and
  `%APPDATA%\Basilica Audio\Overture\Presets\` (Windows). On first launch `PresetManager` copies
