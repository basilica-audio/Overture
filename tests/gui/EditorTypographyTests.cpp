...
// rollout-2026-07/overture/out/VERDICT.md), so every engraved label is a
...
TEST_CASE ("Engraved labels brighten the dark plate where the raw render is clean", "[gui][typography]")
{
    ...
    const auto plate = juce::ImageCache::getFromMemory (BinaryData::plate_overture_png,
                                                        BinaryData::plate_overture_pngSize);
    ...
    CHECK (labelledControls == 11); // every control carries an engraved label
}
...
TEST_CASE ("PlateTypography renders glyphs and its offset pass on a flat ground", "[gui][typography]")
{
    ...
    juce::Image canvas (juce::Image::RGB, 160, 24, true);
    {
        ...
        typography.drawEngraved (g, "OVERSAMPLE", canvas.getBounds().toFloat(), 1.0f, style);
    }
