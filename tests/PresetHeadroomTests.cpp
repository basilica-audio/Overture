...
// The factory-preset headroom gate (issue #44).
//
...
namespace
{
    ...
    // What the shipped output trims in presets/factory/*.json AIM at, which is
    // 0.3 dB below the asserted line. The gap is deliberate: it is the
    // difference between "an upstream voicing tweak moved the peak a little"
    // and "this gate goes red". A trim is derived as (measured overshoot +
    // 0.3 dB), rounded up to the output parameter's 0.01 dB step - never
